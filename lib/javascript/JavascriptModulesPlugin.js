 ...
 * @property {SyncBailHook<[Module, RenderBootstrapContext], boolean>} canInlineInRuntime
 * @property {SyncBailHook<[Module, RenderContext], boolean>} canEmbedInRuntime
	...
	static getCompilationHooks(compilation) {
		...
		if (hooks === undefined) {
			hooks = {
				...
				canInlineInRuntime: new SyncBailHook(["module", "renderContext"]),
				canEmbedInRuntime: new SyncBailHook(["module", "renderContext"]),
			...
			for (const m of inlinedModules) {
				...
				if (renderedModule) {
					...
					const iife =
						...
						hooks.canEmbedInRuntime.call(m, renderContext) === false;
					let footer;
					if (iife) {
		...
		if (!runtimeRequirements.has(RuntimeGlobals.startupNoDefault)) {
			if (chunkGraph.getNumberOfEntryModules(chunk) > 0) {
				...
				for (const entryModule of chunkGraph.getChunkEntryModulesIterable(
					...
					if (
						result.allowInlineStartup &&
						hooks.canInlineInRuntime.call(entryModule, renderContext) === false
					) {
						buf2.push("// This entry module can't be inlined");
						result.allowInlineStartup = false;
					}
					i--;
