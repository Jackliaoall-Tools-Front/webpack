...
class AbstractLibraryPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap(_pluginName, compilation => {
			...
			if (
				this.canEmbedInRuntime !==
				AbstractLibraryPlugin.prototype.canEmbedInRuntime
			) {
				hooks.canEmbedInRuntime.tap(_pluginName, (module, renderContext) => {
					const options = getOptionsForChunk(renderContext.chunk);
					if (options === false) return;
					return this.canEmbedInRuntime(module, renderContext, {
						options,
						compilation
					});
				});
	 ...
	 */
	canEmbedInRuntime(module, renderContext, libraryContext) {
		return undefined;
	}
